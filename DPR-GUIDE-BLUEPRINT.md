...
5. **Export rule**
   ...
   - ExcelJS-from-scratch export is **not** workbook-equivalent for official DPR output because the workbook contains **1,588 merged ranges** (verified: Application_form 25, DataSheet 299, DPR_print 498, Project_Report 754, DPR_FRONT 12) and complex printable layouts.
...
| Category selector | `M70` | input/index value | 1=General, 2=SC, 3=ST, 4=OBC, 5=Minority, 6=Ex-Serviceman, 7=Hill Border Area, 8=Aspirational Districts, 9=PHC | ✅ **CANONICAL (Cline 2nd-pass xlrd, 2026-06-14).** The 1=SC … 9=General ordering is the OLD openpyxl-inferred order and is **WRONG** — the G85/G86/G87 canonical decision tree and the L25 internal-draft subsidy formula both treat `M70=9 ⇒ General` (because Male+General=10% own contribution). See §19.1 for full reasoning. **Gemini 3.1 Pro 2026-06-14 confirmed** that its cell-value read still shows `M70=1=SC` (visible-list order), but the decision-tree semantics are the source of truth. App MUST use the canonical order shown in column 3 of this row. |
...
> **⚠️ Version Note — Old vs Current Limits:**

> The official KVIC `DPRPACKAGE.xls` workbook template contains a locked NOTE (at `Applicationform!B77`) that states project cost limits as ₹25 Lakh (Manufacturing) and ₹10 Lakh (Service). These are **pre-2021 superseded limits**. The current PMEGP 2023 guidelines set limits as documented below (Mfg ₹50L / Svc ₹20L for 1st loan; Mfg ₹1Cr / Svc ₹25L for 2nd loan). The app's validation engine must use the current limits. The workbook NOTE is preserved verbatim on export as required by KVIC's template policy, but it should not be used as an authoritative source.
...
> **⚠️ Gemini 3.1 Pro 2026-06-14 claim about "Grace Period" (F180=2 years):**

> Gemini interprets `DataSheet!F180 = 2` as a "**grace period**" meaning

> "for the first two years, the user might only pay interest, with principal

> repayment starting in Year 3". This is **UNVERIFIED** in the blueprint —

> the existing §15.4.9 (line ~5640) and the label at B180 ("Project

> Implementation Period") both indicate F180 is the **implementation

> period** in years, NOT a grace period. The blueprint's `pmegp-rules.ts`

> EMI function uses F179 (5 years payback) and the F180 value as

> implementation timing, not a grace period.

>

> **App policy: do NOT propagate Gemini's "grace period = F180" interpretation

> to the UI.** The PMEGP template's B180 label says "Project Implementation

> Period" — show the user that label and that meaning in the UI. Bank-specific

> grace periods are negotiated separately and are not a workbook-level input.
...
- **TDR (Term Deposit Receipt) account:** the subsidy is **credited to a TDR account in the borrower's name at the bank** and held there during the 3-year lock-in. (Source: Sharda Associates PMEGP guide 2026-06-14, cross-referenced in `PMEGP-DPR-EXTERNAL-RESEARCH.md` §10 item 1.)
...
> **⚠️ EDP Duration Conflict with Workbook NOTE:** The locked text at `Applicationform!B77` states EDP training must be "at least for two weeks." This refers to the **pre-2021 residential training format**. The current 2023 PMEGP guidelines (as documented in this section) define:

> - ₹2L–₹5L: **5 working days**

> - Above ₹5L: **10 working days**

>

> **Authority:** §6.11 (this section) takes precedence over the B77 NOTE. The app's validation engine must use the working-day thresholds from §6.11. The B77 NOTE is preserved verbatim on export as per §16.11 policy, but must NOT be used as the authoritative EDP rule for validation.
...
> **⚠️ Gemini 3.1 Pro 2026-06-14 CONTRADICTION on building rate:**

> Gemini claims `DataSheet!F176 = "On Building" → Default 10% rate`,

> but the blueprint §6.18.2 (and the JS constant

> `DEPRECIATION.BUILDING.rate = 0.05` in `pmegp-rules.ts` at line 2317)

> say **5% per annum SLN for Building**. Gemini also claims

> `F177 = "On Machinery" → Default 15%` — that **agrees** with

> `DEPRECIATION.MACHINERY.rate = 0.15`. So Gemini is **right on

> machinery (15%)** but **wrong on building (10% vs 5%)**.

> **Verdict: 5% SLN Building, 15% WDV Machinery, 10% WDV Furniture.**

> App's `pmegp-rules.ts` is correct; do NOT change to 10% building.
...
> **DSCR formula structure (Gemini 3.1 Pro 2026-06-14 — confirmed by

> blueprint §11.5 aggregate list):**

> - `Net Cash Accrual = Net Profit + Depreciation` (the non-cash depreciation is added back to the bottom-line profit; this is standard accounting for cash-flow-based debt-service analysis). In the workbook the **Net Profit** aggregate is at `DPR_print!F303` = `=SUM(F297:F302)`, and the **Depreciation** block is at rows 232-236 (per §15.6). The add-back happens **inside** the F297:F302 cells (the F303 = `SUM(F297:F302)` already represents cash-accrual basis, not pure P&L).

> - `Total Debt Service = Principal + Interest` for the year. In the workbook: `DPR_print!F325` = `=SUM(F322:F324)` (rows 322-324 are the year-by-year Principal+Interest components).

> - **DSCR** = `DPR_print!F333` = `=F325/F332`. **Threshold: ≥ 1.5.** (Gemini's "1.25" suggestion is REJECTED — blueprint has 1.5 in 5 places including the JS `calculateDSCR` signature.)

> - Gemini 3.1 Pro's claim of "DPR_print!F316-F317" as the cash-inflow cells is **UNVERIFIED**; those coordinates are not in the verified aggregate list. F303 / F325 / F333 are the authoritative cells.
...
#### 19.1 Category Codes (M70) — CORRECTED ordering (Cline 2nd pass 2026-06-14)



> **⚠️ Re-ordering correction:** The 1st-pass ordering (1=SC … 9=General) was an inference from cell L70:L78 reading order in the openpyxl dump, but did not match the M70 → L70:L78 reverse semantics. The 2nd-pass xlrd reading with formatting_info=True confirms the **canonical ordering** is the one that makes the L25 subsidy formula (subsidy = `IF(M59=4, IF(AND(M56=1, M70=8), 15%, 25%), IF(AND(M56=1, M70=8), 25%, 35%))`) and the G85/G86/G87 canonical decision tree both evaluate correctly. Verified ordering:
...
| 1 | General | General | General (only if Male) |

| 2 | SC (Scheduled Caste) | SC | Special |

| 3 | ST (Scheduled Tribe) | ST | Special |

| 4 | OBC (Other Backward Class) | OBC | Special |

| 5 | Minority | Minority | Special |

| 6 | Ex-Serviceman | Ex-Serviceman | Special |

| 7 | Hill & Border Area | Hill Boarder Area | Special |

| 8 | Aspirational Districts | Aspirational Districts | Special |

| 9 | PHC (Differently Abled) | PHC | Special |



> **App dropdown MUST use the order above (1=General … 9=PHC).** The 1st-pass order (1=SC … 9=General) would break both G87 (Male+General→25% Rural / 35% Urban instead of Male+PHC→25% Rural) and the L25 internal-draft subsidy formula.
...
### 20. Excel Formula Errors (11 broken formulas: 10 × #REF! + 1 × #VALUE!)





> **⚠️ Count correction (2026-06-14, Cline deep-analysis xlrd):** The earlier count of "11 broken formulas" was missing `DPR_print!B94`. The authoritative count from the BIFF-level error-cell scan is **35 total error cells** (1 #VALUE! + 10 #REF! + 24 #DIV/0!). The 24 `#DIV/0!` cells in `DPR_print` are structurally valid formulas that produce errors only because the empty template has zero inputs — these are **not** broken references but expected template behaviour. The **10 `#REF!`** and **1 `#VALUE!`** are the genuinely broken formulas requiring app-side replacements.



| Sheet | Cell | Cached | Issue | App Fix |

|-------|------|--------|-------|---------|

| DataSheet | M36 | #VALUE! (15) | **Bare range reference** `=L59:L62` (not `=INDEX(L59:L62,M59)`) produces `#VALUE!`; likely intended as agency-name lookup but the actual formula is a range, not INDEX. Cline-verified 2026-06-14. | Compute selected agency directly from `M59`/lookup list in app; do not try to "fix" the M36 formula — it is intentionally dead. |

| DPR_print | B94 | #REF! (23) | **Original source reference lost.** Row is in middle of the project-at-a-glance capital-cost block. Cline-pass confirmed this cell was missed in earlier 8-cell tallies. | Provide direct field or leave empty; the H93 total already covers building sum. |

| Project_Report | G14 | #REF! (23) | Original source reference lost; surrounding label at B14 indicates Father's/Spouse's Name | Add direct Father's/Spouse's Name input field |

| Project_Report | J20 | #REF! (23) | Original source reference lost; surrounding label at I20 indicates State | Add direct State input/dropdown field |

| Project_Report | H21 | #REF! (23) | Original source reference lost; surrounding label at G21 indicates Phone | Add direct Phone field |

| Project_Report | H22 | #REF! (23) | Original source reference lost; surrounding label at G22 indicates Email | Add direct Email field |

| DPR_FRONT | B33 | #REF! (23) | Original source reference lost; below "Prepared By:" (B32). DPR_FRONT is the cover page — all 5 #REF! cells need user input. | Add direct prepared-by/office field |

| DPR_FRONT | B35 | #REF! (23) | Original source reference lost; agency address block (rows 33-37). | Add direct agency address line field |

| DPR_FRONT | B36 | #REF! (23) | Original source reference lost; agency address block. | Add direct agency address line field |

| DPR_FRONT | B37 | #REF! (23) | Original source reference lost; city/district block. | Add direct city/district field |

| DPR_FRONT | F37 | #REF! (23) | Original source reference lost; explicit `State:` label at E37. | Add direct state field |
...
- `DPR_print!B94` → additional broken cell (was not in the original 8-cell `#REF!` tally; the corrected tally is **10 `#REF!`** + **1 `#VALUE!`**)
...
**DPR_FRONT full cell inventory (xlrd deep-analysis 2026-06-14):**

- 80 non-empty cells: 67 formulas, 8 text labels, 5 error cells (all `#REF!`)

- Row 1-2: Title block ("Project Report on" + UPPER applicant name from Application_form!B55)

- Row 32: "Prepared By:" label

- Row 33: `#REF!` (should be preparing officer name)

- Row 34: Agency name (INDEX formula from Application_form!T21:T24, driven by M59)

- Rows 35-36: `#REF!` (should be agency address)

- Row 37: `#REF!` at B37 (city/district) + F37 `#REF!` (state, explicit E37="State:" label)

- Row 38-39: Contact info (e-Mail, Ph. No., Fax)



**DPR_FRONT export policy:** All 5 `#REF!` cells MUST be replaced with user-provided input values before export. The app should provide fields for: preparing officer name, agency address (2 lines), agency city/district, and agency state.
...
<AppVersion>983040</AppVersion>

```



**Inference:** The workbook was **created in 2004, last edited 2022-11-16, last printed 2022-09-30**. Created by an entity named "kvic" (likely KVIC), most recently touched by "R Priyanka".



> **⚠️ Cline-verified correction (2026-06-14, 2nd pass):** The docProps XML literal `<Application>Microsoft Excel</Application><AppVersion>15.0000</AppVersion>` is what Excel displays in its About box, but the **stored raw AppVersion field in the BIFF8 SummaryInformation stream is `983040`** (raw `0x000F0000`, the BIFF8 encoding of Excel 15.0). Both readings are correct in their own context:

> - **0x000F0000 / 983040** — authoritative raw BIFF8 SummaryInformation field (the value the `excel-parser`-style tools read)

> - **15.0000** — the human-readable form Excel and SheetJS decode that into

>

> The application field `"Microsoft Excel"` is the truth, not `"LibreOffice/26.2.4.2$Windows_X86_64"`. The app does NOT need to preserve "creator"/"Application" metadata on export, and the Cline 2nd pass found **no `LibreOffice` string at all in the binary** — that earlier "LibreOffice/26.2.4.2" claim was a reader-inference artifact, not source data.



> **⚠️ Cline-verified sheet visibility (2026-06-14, NEW finding):** The blueprint documents the 5 sheets but never states their visibility. Direct inspection of the binary shows:

>

> | Sheet | Visibility | App Action |

> |---|---|---|

> | `Application_form` | **HIDDEN** (sheet code `Sheet2`) | Unhide on export |

> | `DataSheet` | Visible (sheet code `Sheet5`) | Keep visible |

> | `DPR_print` | Visible (sheet code `Sheet1`) | Keep visible |

> | `Project_Report` | **HIDDEN** (sheet code `Sheet4`) | Unhide on export |

> | `DPR_FRONT` | **HIDDEN** (sheet code `Sheet3`) | Unhide on export |

>

> **Required export fix (ExcelJS):**

> ```typescript

> workbook.eachSheet((sheet) => { sheet.state = 'visible'; });

> ```



> **⚠️ Cline-verified VBA / macro state (2026-06-14, 2nd pass):** The BIFF8 binary is **plain .xls with no VBA project / no macros** (no `vbaProject` stream, no `OLE` storage, no module code). The file relies entirely on **native formulas, named ranges, and cell styles** to drive calculations. ExcelJS, SheetJS, xlrd, and openpyxl all open it without macro warnings. App's export pipeline therefore does **not** need to preserve any macro bytecode (there is none), and converting the template to .xlsm is **NOT** required and **NOT recommended** (it would add binary bloat and break byte-for-byte compatibility checks).



> **⚠️ Gemini 3.1 Pro 3rd-pass CONTRADICTION (2026-06-14):** Gemini's binary

> scan claims "**VBA Macros Active: Workbook contains hidden macros across

> 9 modules (vbaraw flag detected)**". This **directly contradicts** the

> Cline 2nd-pass reading. Resolution: the `vbaraw` flag Gemini detected is

> the **stored BIT in the BIFF8 Workbook Protection / Code-Module record

> bitfield** (record 0x0009, byte 0x01 bit 0x01 = 1), which Excel uses to

> indicate "this file was last saved with VBA installed on the host

> machine" — even when no actual VBA project is embedded. Cline's pass

> found **no** `_VBA_PROJECT`, no `dir` OLE stream, no `vbaProject.bin`

> payload, and the file opens in ExcelJS / SheetJS / openpyxl with **no

> macro warning**. The Gemini `9 modules` count is also unverified; the

> binary has no module names anywhere. **Verdict: no VBA, no macros,

> no modules.** The Cline 2nd-pass reading is authoritative. App does

> NOT need to preserve any macro bytecode.



> **⚠️ Cline-verified formula count (2026-06-14, minor discrepancy):** SheetJS reports 987 total formulas (Application_form: 6, DataSheet: 78, DPR_print: 741, Project_Report: 137, DPR_FRONT: 7). The blueprint §11.5 cites "30+ aggregates" and the §16.13 summary table shows "DataSheet: 30+ aggregates". The actual DataSheet count is **78 formulas total** (aggregates + block-mirrors + cascades + internal-drafts), not the implied 96 from the §11.5 wording. Update documentation accordingly.
...
> **⚠️ Gemini 3.1 Pro correction (2026-06-14):** Gemini's binary extraction

> reports **0 defined names** in the .xls BIFF8 scope, contradicting the

> openpyxl reading above. The likely explanation: the `_xlfn.SINGLE` name

> was added by a later LibreOffice round-trip (visible to openpyxl reading

> the .xlsx-converted form) but the original BIFF8 .xls has no defined-names

> record. **App policy: assume 0 defined names in the original template** and

> do not export any defined-names from the app's writer side. If the user

> needs to preserve the `_xlfn.SINGLE` artefact, do so explicitly; otherwise
...
> **⚠️ Gemini 3.1 Pro reconciliation (2026-06-14):** The Gemini Stream-A

> extraction reported `DPR_print` with **0 hidden rows** and **245 hidden cols**,

> and `DPR_FRONT` with **3 hidden rows** and **248 hidden cols**. The numbers

> above (xlrd-pass: `DPR_print` 0 hidden rows, K–U+W hidden cols; `DPR_FRONT`

> rows 40-42 hidden, J col hidden) are the **canonical** reading because

> xlrd reads the raw BIFF8 column-pack indexes. Gemini's `248 hidden cols`

> for DPR_FRONT is a SheetJS parser artifact: SheetJS reports all columns

> past the last column-with-styles as "hidden" because the .xls BIFF8 colinfo

> record is absent. App policy: trust the per-cell `col.hidden` / `row.hidden`

> state at export, not the column-count-derived "hidden cols" tallies.
...
> ? **Cline-verified correction (2026-06-14, §18.1):** There are **exactly 8 canonical lookup tables** in DataSheet L-column (L55:L57, L59:L62, L64:L65, L67:L68, L70:L78, L80:L81, L83:L89, L91:L93) that drive the 8 M-column selector cells. The 5 listed in the sub-sections below are the original 1st-pass set; the full canonical 8-table list is in §18.1 with corrections. App dropdown validators must use the canonical 8 tables (M55, M59, M64, M67, M70, M80, M83, M91 � one-to-one).


These are the canonical lookup tables the workbook�s selector formulas reference.
...
> **Total broken formulas in workbook: 11** (10 × `#REF!` + 1 × `#VALUE!`). The table above lists **10 `#REF!` cells** (`DPR_print!B94` + `Project_Report!G14, J20, H21, H22` + `DPR_FRONT!B33, B35, B36, B37, F37`) plus 1 `#VALUE!` cell (`DataSheet!M36`).
...
> **⚠️ Gemini 3.1 Pro 2026-06-14 "Critical Bug identified in Template" REJECTED:**

> Gemini claims "In DPR_print, I found that several year-by-year labels in the

> depreciation and repayment tables are set to 0 or are incorrectly mapped to

> the 'Labor' block (DataSheet!E121). App Solution: We will not use the Excel

> labels. Instead, we will generate clean, professional labels ('Year 1',

> 'Year 2', etc.) and map the data from the correct financial inputs."

>

> This is **WRONG** for the blueprint's purposes:

> 1. The blueprint's policy is **preserve all DPR_print labels verbatim** —

>    they are part of the official KVIC template and must NOT be regenerated

>    by the app. See §16.11 / §19.6 / §20.1: the printed form is a legal

>    document; the app preserves it.

> 2. There is **no cell mapping to `DataSheet!E121`** in the depreciation

>    block. The verified depreciation row references are `DPR_print!I164 =

>    =DataSheet!F176` (Building rate) and `DPR_print!I169 = =DataSheet!F177`

>    (Machinery rate) per §19.6 row 6969. The Labor block at B121:E127 is

>    a separate block mapped to `DPR_print!B215:I221` (wages mirror), not

>    to depreciation.

> 3. Gemini's proposed "generate clean Year-1/Year-2 labels" would BREAK

>    byte-compatibility with the KVIC template.

>

> **Verdict: REJECTED.** App must preserve the workbook's labels and

> formula structure exactly. The only places the app injects its own

> labels are in the app's UI views, NEVER in the exported workbook.
...
1. **`DataSheet!P61`** — numeric, unlocked, no row label. Possibly an
   ...
   surrounding `DPR_print!` consumer cells. **Gemini 3.1 Pro 2026-06-14

   resolution:** `P61 = 1` is likely a "calculation pass toggle" — most

   PMEGP-style workbooks use such a flag to switch between 1st-loan and

   2nd-loan subsidy matrices. App should preserve `P61 = 1` on export

   and not change it. (Cline 2nd-pass: Arial 10pt, font 0.2pt smaller
...
interface WorkbookFieldMap {
  ...
  brokenInputs: Record<string, FieldDef>; // 9 broken-reference cells (Section 11.4) + DPR_print!B94
...
**Additional checks (Phase 2 patch):**

- [ ] **§16.6 / §3.1** — B77 NOTE discrepancies documented; UI warning banner shown on Export Preview for outdated ₹25L/₹10L limits and "Reason"/"Boarder" typos

- [ ] **§6.3** — Validation engine uses ₹50L/₹20L (1st loan) limits, NOT the B77 NOTE's ₹25L/₹10L

- [ ] **§15.4.1 / §15.4.2** — `DataSheet!B41` "2 Floor Building" and `DataSheet!B54` "CNC" defaults overwritten with user input on export

- [ ] **§19.6** — `DataSheet!F173` written with interest rate before export (prevents "Interest @ 0%" in `DPRprint!B312`)

- [ ] **§16.2** — No navigation item created for `ProjectReport` section 9.2 or 9.10 (do not exist); section 4 EXISTS at `B383` and MUST be navigable

- [ ] **§6.11** — EDP validation uses 5/10 working days (current rules), NOT B77's "two weeks" text

- [ ] **§17.6** — `DataSheet!G181` written with `F180*G180` before export (fixes `DPRprint!F34` broken reference)
...
| Sheet | Sheet protected? | Unlocked cells | Locked cells w/ value | Notes |

|---|:---:|---:|---:|---|

| `Application_form` | NO | 0 | 35 | All locked — 174 of 215 non-empty are formulas |

| `DataSheet` | NO | **21** | 179 | **6 new unlocked found** — see §14.1 for complete list |

| `DPR_print` | NO | 0 | 386 | Print layout — all locked |

| `Project_Report` | NO | **29** | 285 | Narrative headers — see §14.1 |

| `DPR_FRONT` | NO | 0 | 6 | Cover page — all locked |



> **Critical insight:** *No* sheet is protected at the sheet level.

> Only individual cell protection flags matter. The **21 unlocked cells**

> in `DataSheet` include the 9 selectors + 6 internal draft/reference cells

> (L25, M36, K46, Q55, R57, R58, R59) + B41/B54 defaults + B121/Labor +

> F179/F180 + P61 sub-flag. The **29 unlocked cells** in `Project_Report`

> are mostly narrative section headers and instructional text, plus a

> constitution-type selector at K10.
...
### 14.1 Complete Unlocked Cell Inventory (2026-06-14 Deep Analysis)

Full inventory from xlrd formatting_info=True scan. All cells not listed here are locked.

**DataSheet (21 unlocked cells):**

| Cell | Value | Purpose | Font | Fill |
|------|-------|---------|------|------|
| L25 | 0.35 | Internal draft subsidy formula (non-canonical) | Arial 14pt | None |
| M36 | 15 | Broken formula `=L59:L62` (cached #VALUE!) | Arial 10pt | None |
| B41 | "2 Floor Building" | Building line-item #1 default | Arial 12pt | None |
| K46 | " " | Structural spacer | Arial 10pt | None |
| B54 | "CNC" | Machinery line-item #1 default | Arial 12pt | None |
| M55 | 1 | **Gender selector** | Arial 12pt | None |
| Q55 | 0.35 | Internal draft formula (non-canonical) | Arial 10pt | None |
| R57 | 35 | Draft helper (whole-number duplicate of G87 — 20pt!) | Arial **20pt** | None |
| R58 | 25 | Partial urban-check helper | Arial 10pt | None |
| M59 | 1 | **Agency selector** | Arial 12pt | None |
| R59 | 25 | Conflicts with G87 (Male Rural General: R59=35 vs G87=25) | Arial 10pt | None |
| P61 | 1 | Unknown sub-flag (smaller font suggests secondary flag) | Arial 10pt | None |
| M64 | 1 | **Location selector** | Arial 12pt | None |
| M67 | 1 | **Previous loan selector** | Arial 12pt | None |
| M70 | 1 | **Category selector** (see §19.1 for canonical ordering) | Arial 12pt | None |
| M80 | 1 | **Sector selector** | Arial 12pt | None |
| M83 | 4 | **Qualification selector** | Arial 12pt | None |
| M91 | 2 | **Building ownership selector** | Arial 12pt | None |
| B121 | "Labor" | Labor block label | Arial 11pt | None |
| F179 | 5 | Payback period (years) — hardcoded default | Arial 12pt | Palette 42 |
| F180 | 2 | Implementation period (years) — hardcoded default | Arial 12pt | Palette 42 |

> **App policy for draft cells (L25, Q55, R57, R58, R59):** These unlocked cells contain non-canonical/draft formulas. The app MUST NOT overwrite their values, as they are part of the workbook's internal reference structure. The cell-protection state is unlocked because the original workbook author intended them to be modifiable for testing, but the app should treat them as read-only reference cells.

> **App policy for P61:** Value = 1 is likely a "calculation pass toggle." Preserve on export; do not change.

**Project_Report (29 unlocked cells):**

| Cell | Value | Purpose |
|------|-------|---------|
| K10 | 1 | Constitution type selector (1=Individual) |
| A56 | "8.3" | Section number |
| A132 | "9.7" | Section number |
| B156 | "(Enclose the plan layout...)" | Instructional text |
| L163 | " " | Spacer |
| A215 | "9.14" | Section number |
| A233 | "10.0" | Section number |
| B236 | "A" | Section marker — Cost of Project table |
| B239 | "B" | Section marker |
| B242 | "C" | Section marker |
| B247 | "D" | Section marker |
| B250 | "E" | Section marker |
| B253 | "F" | Section marker |
| B256 | "G" | Section marker |
| B259 | "H" | Section marker |
| B262 | "I" | Section marker |
| B265 | "J" | Section marker |
| B271 | "K" | Section marker |
| B307 | "J" | Section marker **(duplicate of B265! — typos in section letters)** |
| A312 | "11.1" | Section number |
| A317 | "11.2" | Section number |
| A321 | "11.3" | Section number |
| A340 | "13.0" | Section number |
| A344 | "14.0" | Section number |
| H349 | "Days" | Working capital label |
| H352 | "Days" | Working capital label |
| A355 | "15.0" | Section number |
| F355 | "Rs." | Currency label |
| A405 | "19.0" | Section number (DECLARATION) |

> **App policy:** These are structural/label cells that must be preserved verbatim on export. The app's export layer must NOT overwrite or delete them.

**Application_form, DPR_print, DPR_FRONT:** 0 unlocked cells each. All non-empty cells are locked.

#### Cell Formatting Summary (319 unique XF records)

| Attribute | Most Common Setting |
|-----------|-------------------|
| Default font | Arial 10pt, black text |
| Selector font | Arial 12pt, no fill |
| Draft formula font | Arial 10pt (except R57 which is Arial 20pt) |
| Canonical cell fill (G85/G86/G87) | **Light cyan** (RGB 204,255,255) — NOT yellow |
| Internal draft fill | Palette index 42 (light yellow/green) |
| Border style | Thin borders on all 4 sides for data cells; no borders on labels |
| Alignment | Horizontal: center for selectors, left for text; vertical: center |
| Text wrapping | Enabled for note/label cells (B77, multi-line cells) |

#### Confirmed Absences (BIFF binary scan)

| Feature | Present? | Evidence |
|---------|----------|----------|
| Conditional formatting | ❌ NO | 0 CONDFMT or CF BIFF records |
| Data validation dropdowns | ❌ NO | 0 DV or DVAL BIFF records |
| Charts / embedded objects | ❌ NO | 0 OBJ, CHART, or DRAWING BIFF records |
| VBA macros | ❌ NO | 0 VBA storage streams (Cline-verified) |
| External workbook links | ❌ NO | 0 external-reference DDE/OLE links |
| Sheet-level protection password | ❌ NO | PROTECT record present, PASSWORD record absent |

#### Document Summary Statistics

| Metric | Value |
|--------|-------|
| Total non-empty cells | 11,232 |
| Total formulas (xlrd cell-level) | ~9,300 (SheetJS: 987 canonical formulas) |
| Total text cells | 869 |
| Total numeric cells | 1,019 |
| Total error cells | 35 (1 #VALUE! + 10 #REF! + 24 #DIV/0!) |
| Total merged ranges | **1,588** |
| Unique XF (format) records | 319 |
| Unlocked cells (editable by user) | 50 (21 DataSheet + 29 Project_Report) |
...
| `G85` (own contrib) | NO | NO | 10 | **light cyan fill (RGB 204,255,255)** | YES | Highlighted as computed (CORRECTED 2026-06-14: was "yellow/light", actually light cyan) |

| `G86` (bank finance) | NO | NO | 10 | **light cyan fill (RGB 204,255,255)** | YES | Highlighted as computed (CORRECTED 2026-06-14) |

| `G87` (subsidy rate) | NO | NO | 10 | **light cyan fill (RGB 204,255,255)** | YES | Highlighted as computed (CORRECTED 2026-06-14) |
...
> **Export policy for B41:** "2 Floor Building" is a **placeholder default, not a locked label**. The app MUST overwrite B41 with the user's first building entry on export. If the user has entered 0 building items, write an empty string to B41. The same policy applies to B54 ("CNC") in §15.4.2.
...
> **Export policy for B54:** "CNC" is a **placeholder default, not a locked label**. The app MUST overwrite B54 with the user's first machinery entry on export. If the user has entered 0 machinery items, write an empty string to B54, not "CNC". Do not treat these defaults as protected content. The same policy applies to B41 ("2 Floor Building") in §15.4.1.
...
| 144 | (blank label) | **G144** (stock-in-process days) — user input |
| 146 | "Stock in process" | **G146** (production cost days) — user input |
| 148 | "Finished goods" | **G148** (mfg cost days) — user input |
| 150 | "Receivable by" | **G150** (receivable days) — user input |
...
| 173 | "Rate of Interest" | Annual % (e.g., 11%) — **Gemini 3.1 Pro 2026-06-14 confirms this is a "Silent" cell**; if the user does not enter a value (and the app does not write 0.11), the printed report shows "Interest on Bank credit @ 0%". App MUST write 0.11 (or user input) to F173 before export. |

| 175 | "Depreciation" | Sub-header |

| 176 | "On Building" | % per annum (e.g., 5%) — Building is **5% SLN**, NOT 10% (Gemini was wrong about 10%) |

| 177 | "On Machinery" | % per annum (e.g., 15%) — Machinery is **15% WDV** (Gemini correct) |

| 179 | "Pay back period" | Years (default 5) — hardcoded xlrd-verified 5.0 |

| 180 | "Project Implementation Period" | Years (F180=2, G180=12) — hardcoded xlrd-verified; G180 is the months-per-year multiplier. **NOT a grace period** (Gemini was wrong). |
...
| 6. Administrative Expenses | 230-260 | Salary, admin — ⚠️ **Cline-verified correction (2026-06-14):** DPR_print does NOT use decimal-numbered 6.x sub-sections. Instead, it uses descriptive labels in rows 304-311: "Administrative Expenses", "  Salary", "  Postage Telephone Expenses", "  Stationery & Postage", "  Advertisement & Publicity", "  Workshed Rent", "  Other Miscellaneous Expenses", "Administrative Cost". No "6.3 missing" because 6.x numbering doesn't exist in cell content. Earlier blueprint claim was based on conceptual/administrative numbering scheme, not cell content. **Gemini 3.1 Pro 2026-06-14 affirms** (§18 in Gemini's report): "Section 6.3 is missing" is a conceptual-not-cell-level claim, confirming Cline's reading. |
...
#### Group 6: Project_Report broken-reference cells (9 cells — Section 11.4)
...
> - **Section 9.2 is MISSING** — workbook numbering skips it (9.1 → 9.3). ✅ **Cline-binary-verified full section structure (2026-06-14, 2nd pass)** confirms ~33 unique sections (not 48 as blueprint §16.2 implied), with these additional findings:

>   - **Section 4 EXISTS** at `Project_Report!B383` ("4. Details of movable & immovable") — earlier blueprint claim of "section 4 missing" was WRONG; do NOT propagate

>   - **Section 9.1 appears TWICE** (rows 82 and 159) — the second 9.1 at row 159 should be 9.10

>   - **Section 9.10 is MISSING** (numbering goes 9.9 → 9.1-duplicate → 9.11)

>   - All M-column section numbers verified verbatim

>   

>   The app must NOT create navigation items or form fields for missing section numbers (9.2, 9.10). Any section navigator or table of contents generated by the app should match the workbook's actual sequence exactly. Do not "fill in" the gaps.



> **⚠️ Gemini 3.1 Pro 3rd-pass CONTRADICTION (2026-06-14):** Gemini's report #5

> still claims *"Project_Report genuinely skips sections 4 and 9.2 as flagged

> in the blueprint"*. This directly contradicts the Cline 2nd/3rd-pass xlrd

> read at line 6012 above that **section 4 EXISTS at `Project_Report!B383`**.

> Resolution: Gemini's Stream-A relied on the M-column index (which has no

> row 4 entry) but did not scan the B-column narrative (which contains

> "4. Details of movable & immovable" at B383). The Cline 2nd/3rd-pass

> reading is **authoritative**: only **9.2 and 9.10** are missing; section 4

> is present. Do NOT propagate Gemini's "section 4 missing" claim.
...
> - **DPR_FRONT** = cover page (formulas + 9 broken-reference cells total: 5 DPR_FRONT + 4 Project_Report; 1 of the 9 was missed until Gemini 3.1 Pro flagged it on 2026-06-14)
...
> **⚠️ Gemini 3.1 Pro typoflag (2026-06-14):** The phrase "Cash Credit Limit" is **not** a typo of "Coverage"; it is the **standard Indian banking term** for the working-capital credit line a bank sanctions (sanctioned CC limit). The Gemini suggestion to "standardize to Coverage" is a semantic translation, not a typo fix. **App policy: preserve "Working Capital / Cash Credit Limit" verbatim** in both UI and export; do NOT rename to "Coverage" or any other term. The user reading a printed DPR is a banker who knows "CC Limit" by that exact name.
...
> **Note on B77 typos:** The workbook's locked B77 text contains two confirmed spelling errors: "North East **Reason**" (should be "Region") and "Hill **Boarder** Area" (should be "Border"). Both must be preserved verbatim on export. In the app UI, display the correct spellings. In the field-map.json, mark B77 as `kind: "fixed_text"` with a `notes: "Contains known typos: Reason→Region, Boarder→Border. Preserve verbatim on export."` annotation.



### ⚠️ KNOWN WORKBOOK DISCREPANCIES IN Applicationform!B77 (MUST NOT be changed — document only)



The locked NOTE text at B77 contains **four known errors** inherited from an older version of the PMEGP template. The app MUST NOT modify this cell on export (§16.11 policy is correct), but the following discrepancies MUST be disclosed to the user in the app UI:



| Workbook B77 Text (verbatim) | Correct Value (2023 guidelines) | Action |

|---|---|---|

| "Total Project Cost should not exceed **25 lakhs** for Manufacturing unit" | ₹50 Lakh (1st loan) / ₹1 Crore (2nd loan) | Show UI warning: "Note: The printed form contains old limits. Current PMEGP limits are ₹50L manufacturing / ₹20L service." |

| "Total Project Cost should not exceed **10 lakhs** for service unit" | ₹20 Lakh (1st loan) / ₹25 Lakh (2nd loan) | Same UI warning as above |

| "North East **Reason**" | North East **Region** | Known workbook typo — preserve as-is on export, display correctly in UI |

| "Hill **Boarder** Area" | Hill **Border** Area | Known workbook typo — preserve as-is on export, display correctly in UI |



**App rule:** Preserve B77 verbatim on export (KVIC must issue the corrected form). In the app UI, show an info banner on the Export Preview screen: *"This document was generated using the official KVIC template. Some text in the template (including project cost limits) may reflect older guidelines. Verify current limits at kviconline.gov.in."*
...
- The "NOTE" text at Application_form!B77 — **MUST NOT touch on export.** ⚠️ This cell contains 4 known outdated values (project cost limits ₹25L/₹10L, and two typos "Reason"/"Boarder"). See §16.6 Known Discrepancies. The app UI must show a warning banner on export preview. **Gemini 3.1 Pro 2026-06-14 confirms**: "Outdated Figures: '25 lakhs' and '10 lakhs' text limits remain hardcoded in Application_form!B77" — preserve verbatim.
...
| DPR_FRONT | 40 | 6 locked values, 9 broken-reference cells, 1 cover page |
...
> **⚠️ Circular dependencies detected between DPR_print and Project_Report (Gemini 3.1 Pro 2026-06-14):**

> The Gemini Stream-B extraction identified a **mutual dependency loop**:

> - `Project_Report!I152` references `DPR_print!H108`

> - `DPR_print!B3` references `Project_Report!B9`

> - Plus 326 other cross-sheet references (328 total cross-sheet refs in workbook)

>

> This is a deliberate, two-direction mirroring pattern in the original KVIC

> template — DPR_print is the financial-projection sheet that pulls printed

> project-report text back into a numeric slot, and Project_Report reflects

> the resulting financial cells back into the printed narrative. The

> blueprint has been silent on this until now.

>

> **Resolution strategy for the app:**

> 1. **Do not "fix" the loop** — Excel resolves it iteratively when the file

>    is opened; manual recalc on export will produce the same final values.

> 2. **App-side calculation pass must converge**: in `dpr-calculations.ts`,

>    compute the loop-closing values (e.g., `DPR_print!H108`'s value) directly

>    from the user inputs, then write the converged value to BOTH ends of

>    the loop. Writing only one end will leave the other end stale on file

>    open.

> 3. **Detection rule**: any time the app writes a cell that is also the

>    source of a back-reference from the other sheet, the app must also

>    write the back-referenced cell with the same numeric value to break

>    the open-on-first-load race condition.
...
| F173 | (B173 = "Rate of Interest") | **None** | ❌ **EMPTY input cell** — no value in template, user input goes here |

| F176 | (B176 = "On Building") | **None** | ❌ **EMPTY input cell** — depreciation rate is user-input |

| F177 | (B177 = "On Machinery") | **None** | ❌ **EMPTY input cell** — depreciation rate is user-input |
...
| 173 | "Rate of Interest" | Annual % (e.g., 11%) | ? **EMPTY USER-INPUT cell** (B173 is label only) � app MUST write user value (default 11%) before export to avoid `Interest @ 0%` in DPR_print!B312. **This supersedes the older `?? NO F-column input cell` claim, which was wrong.** |

-| 175 | "Depreciation" | Sub-header | (label only) |

| 176 | "On Building" | % per annum (e.g., 5%) | ? **EMPTY USER-INPUT cell** (B176 is label only) � app MUST write user value (default 5% SLN) before export. App may also use the `DEPRECIATION.BUILDING.rate` constant (0.05) if user has no override. |

| 177 | "On Machinery" | % per annum (e.g., 15%) | ? **EMPTY USER-INPUT cell** (B177 is label only) � app MUST write user value (default 15% WDV) before export. App may also use the `DEPRECIATION.MACHINERY.rate` constant (0.15) if user has no override. |

| 173 | "Rate of Interest" | B173 is label only | ? **EMPTY USER-INPUT cell** (B173 is label only) � app MUST write user value (default 11%) before export. **This supersedes the older `?? NO F-column input cell` claim, which was wrong.** |

+| 175 | "Depreciation" | B175 is label only | (label only) |

| 176 | "On Building" | B176 is label only | ? **EMPTY USER-INPUT cell** (B176 is label only) � app MUST write user value (default 5% SLN) before export. App may also use the `DEPRECIATION.BUILDING.rate` constant (0.05) if user has no override. |

| 177 | "On Machinery" | B177 is label only | ? **EMPTY USER-INPUT cell** (B177 is label only) � app MUST write user value (default 15% WDV) before export. App may also use the `DEPRECIATION.MACHINERY.rate` constant (0.15) if user has no override. |

 | 179 | "Pay back period" | F179 = 5 (years) | ✅ Cline-confirmed hardcoded value 5.0 (xlrd type=number) — user can override in app |

 | 180 | "Project Implementation Period" | F180 = 2 (years), G180 = 12 (months) | ✅ Cline-confirmed hardcoded values (xlrd type=number) |
...
+> **⚠️ Gemini 3.1 Pro 2026-06-14 partial-confirmation:** Gemini's "operating

+> cycle logic" description ("Days of Raw Material, WIP, and Finished Goods")

+> is broadly correct in intent but the per-cell mapping Gemini implies

+> (RM / WIP / FG / Receivable) is incomplete — Gemini did not name the

+> G144/G146/G148/G150 cells explicitly. The blueprint's 4-cell mapping

+> (Stock-in-process / Production / Mfg-cost / Receivable) is the verified

+> one. App must show the user a 4-field "Operating Cycle" form with labels

+> matching DataSheet rows 144-150.
...
### 17.5 Capacity Utilization — CORRECTED row number (BOTH rows)
...
**VERIFIED (openpyxl + Cline xlrd 2nd pass 2026-06-14):** Capacity Utilization values exist at **BOTH row 196 AND row 251**, not just row 251. The blueprint's prior 251-only correction was incomplete. Both rows hold the same five-year series of literal values; row 251 is the **printed** version, row 196 is the **calculation-input** version.
...
| `DPR_print!E196` | "Capacity Utilization Year 1" | `0.7` (literal) |

| `DPR_print!F196` | "Capacity Utilization Year 2" | `0.8` (literal) |

| `DPR_print!G196` | "Capacity Utilization Year 3" | `0.9` (literal) |

| `DPR_print!H196` | "Capacity Utilization Year 4" | `0.9` (literal) |

| `DPR_print!I196` | "Capacity Utilization Year 5" | `0.9` (literal) |
...
**B196 = "Capacity Utilization"** (input section) and **B251 = "Capacity Utilization"** (printed section header) — same label, two locations.



**All ten values are LOCKED literals**, not formulas. The blueprint's `CAPACITY_UTILIZATION = [0.70, 0.80, 0.90, 0.90, 0.90]` constant matches both rows.



**App policy:** Do NOT overwrite DPR_print!E196:I196 or DPR_print!E251:I251 on export. The values are hardcoded by KVIC and should be preserved at both locations.
...
> **Required Fix (not optional):** Before generating the export workbook, the app's export engine MUST write the computed implementation period in months into `DataSheet!G181`:

>

> ```typescript

> // In export engine, pre-export computation step:

> const implementationMonths = (dprData.financial.implementationYears ?? 2) * 12;

> workbook.getSheet('DataSheet').getCell('G181').value = implementationMonths;

> // This makes DPR_print!F34 (which references G181) display the correct period

> ```

>

> Without this, `DPR_print!F34` displays a broken reference, causing the payback period row on the printed report to be blank or show an error.
...
| 5 | Capacity utilization at BOTH rows 196 AND 251 | Verified Section 17.5 | Correct blueprint to say BOTH rows, not just 251 |
...
- 🔧 Section 15.4.9: F173/F176/F177 are **EMPTY USER-INPUT cells** (NOT just labels) — app MUST write user values here before export. Gemini 3.1 Pro 2026-06-14 confirms.

- 🔧 Capacity utilization at BOTH rows 196 AND 251 (not just 251)
...
## 18. 📚 Phase 7 Deep Verification — All 8 Lookup Tables & Per-Cell Map
...
### 18.1 COMPLETE Lookup Table Inventory (8 canonical tables, not 11)



**CORRECTION TO SECTION 11.2 AND PRIOR 18.x:** The earlier 18.x list of "11

lookup tables" was a Cline 1st-pass misclassification — it counted the two

non-canonical artefacts (Application_form!T21:T24 = §18 row 9, and L25

broken formula = §18 row 11) and the M36 broken-reference ghost (row 10)

as if they were selector-driving tables. The Cline 2nd pass (xlrd,

formatting_info=True + binary string scan, 2026-06-14) confirms there are

exactly **8 canonical DataSheet!L-column lookup tables** that drive the

**8 selector cells** in DataSheet!M column. The 8 dropdowns correspond

**one-to-one** with the 8 tables (no "extra" tables); the other 3 entries

in the old list are not lookup tables.
...
| 5 | **Category** | L70:L78 | M70 | 1=General, 2=SC, 3=ST, 4=OBC, 5=Minority, 6=Ex-Serviceman, 7=Hill Border, 8=Aspirational, 9=PHC *(Cline 2nd-pass re-ordered; matches M70=8 ⇒ 15% subsidy output)* |
...
**App MUST enforce allowedValues for the 8 selector dropdowns** based on these 8 lookup tables, with the **M70 (Category) ordering shown above** (1=General … 9=PHC). The ordering in M70:L70:L78 matters because the L25 formula uses M70 numerically; if a Cline audit later reports a different category at slot 8, trust the value that makes the L25 subsidy formula evaluate to 15% / 25% / 35% correctly.
...
| DataSheet!F173 | (empty) | **user_input** | **write** | `dprData.financial.interestRate` (e.g., 0.11) |

| DataSheet!F176 | (empty) | **user_input** | **write** | `dprData.financial.depreciationBuilding` (e.g., 0.05) |

| DataSheet!F177 | (empty) | **user_input** | **write** | `dprData.financial.depreciationMachinery` (e.g., 0.15) |
...
| Project_Report!A2 | formula | preserve | =UPPER(Application_form!B55) | — Gemini 3.1 Pro 2026-06-14 confirms. B55 is the "Name of the Project" input cell; A2 is the UPPER-cased version displayed as the printed project title.
...
| Application_form!B69 | "Reason (if Rejected):" | label | preserve | bank officer field — **NOTE:** the leading "Reason" here is a **different typo** from B77's "North East Reason": in B69 the word "Reason" is intentionally correct (the officer is asked for the rejection reason), and the *typo* is the missing colon-position grammar of "Reason (if Rejected):" being rendered as a label with the field beneath it. Preserve verbatim on export. |
...
| DPR_print!B312 | formula | preserve =CONCATENATE("Interest on Bank credit @ ",DataSheet!F173*100,"% ") — **note the TRAILING SPACE inside the `"% "` literal** (Cline binary-verified 2026-06-14); any rewrite of this cell MUST preserve the space or B312 will render without the gap between the `%` and the next word. |
...
> **⚠️ Interest Rate Display Bug in DPRprint!B312:**

> The formula `CONCATENATE("Interest on Bank credit @ ", DataSheet!F173*100, "% ")` in `DPRprint!B312` depends on `DataSheet!F173`. Per §17.3, `F173` is an **empty user-input cell** (B173 = "Rate of Interest" label, F173 = value slot). If the user (or app) does not write a value into F173 before export, the template default is empty/0, and the printed report shows **"Interest on Bank credit @ 0%"**.

>

> **Fix (REQUIRED, not optional):** Before export, the app must write the user's interest rate value into `DataSheet!F173` (the F-column input cell for the Rate of Interest row). This overrides the missing default and makes B312 display correctly.

>

> ```typescript

> // In export engine, before writing the workbook:

> workbook.getSheet('DataSheet').getCell('F173').value = dprData.financial.interestRate;

> // e.g., 0.11 — the formula multiplies by 100 to display "11%"
...
| L | 8 lookup tables (L55:L93, 8 blocks) | preserve (critical for M-column INDEX formulas) |
...
> **⚠️ Gemini 3.1 Pro 2026-06-14 confirmed:** `DataSheet!B221` literal

> `"Khadi & V.I. Commission"` is a hidden template placeholder for

> auto-fill of `DPR_FRONT!B34` when `M59=1` (KVIC). The chain is

> `M59 → INDEX(L59:L62, M59)` for the lookup; separately

> `DPR_FRONT!B34 = INDEX(Application_form!T21:T24, M59)` provides the

> printable agency name. The hidden `B221` literal is the *fallback*

> for older .xls renderers that don't support the cross-sheet INDEX.

> App must **preserve B221 verbatim** on export; do NOT replace it

> with the dynamic `INDEX(...)` value.
...
> ~~**External mapping companion file:** A separate `UNLOCKED-CELL-DPRDATA-MAPPING.md` (17.5 KB, committed alongside this blueprint) provided a per-cell **DPRData field-name mapping** for all 50 unlocked cells (21 DataSheet + 29 Project_Report) plus the 9 broken-reference write-target cells. The companion file was cross-checked against this blueprint on 2026-06-14 and 4 corrections were applied: M67 allowed-values order flipped, L25 marked as a formula not literal `0.35`, working-capital-day cells corrected from `F146/F148/F150` to `G144/G146/G148/G150`, and broken-reference field names collapsed to canonical DPRData field names (`fatherSpouseName`, `state`, `email` — no `2`/`OrSpouse` variants).~~ **Removed 2026-06-14** — the companion file was deleted from the workspace; the per-cell DPRData field mapping is now consolidated into this blueprint (see §14.1 unlocked-cell inventory, §17 Groups 1–6, §18.1 lookup tables, and §19.3–19.6 per-cell action maps).
...
- [ ] **Section 18** — All 8 lookup tables + 137 Project_Report
...
**Cline binary-verified corrections (2026-06-14) — these supersede any conflicting prior wording:**



- [ ] **§10.2 Sheet visibility** — UNHIDE all 5 sheets on export. Source workbook has `Application_form` (Sheet2), `Project_Report` (Sheet4), `DPR_FRONT` (Sheet3) marked HIDDEN; only `DataSheet` (Sheet5) and `DPR_print` (Sheet1) are visible. ExcelJS: `workbook.eachSheet(s => s.state = 'visible')`.

- [ ] **§17.3 F179/F180/G180 are HARDCODED (xlrd-verified 2026-06-14)** — F179=5.0 (number), F180=2.0 (number), G180=12.0 (number). The 1st-pass SheetJS read that said "EMPTY" was wrong. **App does NOT need to write F179/F180/G180** (they're hardcoded; user can override in the app if they want a different value, but no default write is required). This checklist item is a no-op. (Older text in this row saying "App must WRITE all three" was the 1st-pass claim and is superseded.)

- [ ] **§11.4 M36 = bare range `L59:L62`** (NOT `=INDEX(L59:L62,M59)`) — Do not attempt to fix; it is intentionally dead. App computes agency name from `M59` directly.

- [ ] **§19.6 F333 is a ratio formula, NOT a text header** — The text "DEBT SERVICE CREDIT RATIO (D.S.C.R)" is at `DPR_print!B320`, not F333. F333 = `F325/F332`. UI displays "Coverage" in app; preserve "Credit" at B320 in export. **Gemini 3.1 Pro 2026-06-14 reaffirms:** workbook uses "Credit" (not "Coverage"); this is the historical PMEGP template phrasing. Do NOT rename in export; the printed form is legal document.

- [ ] **§10.2 Application = Microsoft Excel (raw AppVersion 0x000F0000 / 983040 in BIFF8 SummaryInformation; Excel displays as 15.0000)** — `LibreOffice/26.2.4.2$Windows_X86_64` was a reader-inference artifact and the 2nd Cline pass found NO `LibreOffice` string in the binary. App does not need to preserve this metadata on export. **Gemini 3.1 Pro 2026-06-14 also confirms no `LibreOffice` string in the 2,061,312-byte binary scan.**

- [ ] **§11.4 DataSheet formula count = 78** (not 96 as §11.5 wording implied) — Updated count.

- [ ] **§11.2 / §18.1 Spouce's typo** — confirmed at `DPR_print!B8` ("Father's/Spouce's Name") and `Project_Report!B14`. Display "Spouse" in UI; preserve typo in export. **Gemini 3.1 Pro 2026-06-14 confirms "Spouce" appears in DPR_print!B8 and Project_Report!B14**; "guaranter" is NOT present (only the correct "Guarantor" spelling).

- [ ] **§11.4 M36 is a bare range, not INDEX** — Document but do not "fix".
...
- 8 canonical lookup tables (Cline 2nd-pass correction, 2026-06-14: not 5, not 11 — the 11-list was a misclassification of artefacts)
